...
import { getDentalclinics } from 'apiSdk/dentalclinics';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<DentalclinicInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/dentalclinics` : null),
    () => getDentalclinics({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Dentalclinic',
      path: isTenantUser ? `/dentalclinics/view/${data?.[0]?.id}` : '/dentalclinics',
      entity: 'dentalclinic',
      service: AccessServiceEnum.PROJECT,
      icon: FiBriefcase,
    },
    { name: 'Patient', path: '/patients', entity: 'patient', service: AccessServiceEnum.PROJECT, icon: FiUserCheck },
